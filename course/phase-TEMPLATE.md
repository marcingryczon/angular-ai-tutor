# Phase {{NUMBER}}: {{PHASE_TITLE}}
*Focus: {{PHASE_FOCUS}}*

## Git Branch: `lesson-{{NUMBER}}-*`

---

### Lesson {{NUMBER}}.1: {{LESSON_1_TITLE}}
- *Objective:* {{LESSON_1_OBJECTIVE}}
- *Branch Name:* `lesson-{{NUMBER}}1-{{LESSON_1_SLUG}}`
- *Topics:*
  - {{TOPIC_1_1}}
  - {{TOPIC_1_2}}
  - {{TOPIC_1_3}}
- *Training Exercise:* {{EXERCISE_1}}
- *Project Application:* {{APPLICATION_1}}

---

### Lesson {{NUMBER}}.2: {{LESSON_2_TITLE}}
- *Objective:* {{LESSON_2_OBJECTIVE}}
- *Branch Name:* `lesson-{{NUMBER}}2-{{LESSON_2_SLUG}}`
- *Topics:*
  - {{TOPIC_2_1}}
  - {{TOPIC_2_2}}
  - {{TOPIC_2_3}}
- *Training Exercise:* {{EXERCISE_2}}
- *Project Application:* {{APPLICATION_2}}

---

### Lesson {{NUMBER}}.3: {{LESSON_3_TITLE}}
- *Objective:* {{LESSON_3_OBJECTIVE}}
- *Branch Name:* `lesson-{{NUMBER}}3-{{LESSON_3_SLUG}}`
- *Topics:*
  - {{TOPIC_3_1}}
  - {{TOPIC_3_2}}
  - {{TOPIC_3_3}}
- *Training Exercise:* {{EXERCISE_3}}
- *Project Application:* {{APPLICATION_3}}

---

## Phase Completion Criteria

Before marking this phase as complete:

- [ ] All lessons implemented and merged to `main`
- [ ] All training exercises completed
- [ ] All project applications integrated into TaskFlow
- [ ] Code reviewed and follows best practices
- [ ] Tests pass (if Testing Phase already completed)

---

## Key Takeaways

After completing this phase, the learner should understand:

- {{TAKEAWAY_1}}
- {{TAKEAWAY_2}}
- {{TAKEAWAY_3}}
